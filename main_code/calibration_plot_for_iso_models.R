...
for (mode in mode_list){
  ...
  for (n in 1:length(outcome_list)){
    ...
    ggplot(plot_df_w_sd_2, aes(x=Decile, y=mean_risk, fill=Category)) + 
      geom_bar(stat="identity", color="black", 
               position=position_dodge()) +
      geom_errorbar(aes(ymin=mean_risk-sd, ymax=mean_risk+sd), width=.2,
                    position=position_dodge(.9))  + 
      ylab('Mean risks') + ggtitle(outcome) + 
      theme_minimal() +
    ...
    ggsave(filename = paste0('Cali_plot_',outcome,'.tif'), path = save_dir
           , units = 'in', width = 6.6, height = 5.5
           , device = 'tiff', dpi = 300)
