...
data_preop <- read.csv(paste0(work_dir,'/csv_files/feature_space_preoperative.csv'))
data <- read.csv(paste0(work_dir,'/csv_files/feature_space_pre_intra_anat.csv'))

data$X <- NULL
data<- data %>% dplyr::select(-one_of(c("datavrsn", "patid", "recordid", "aortproc"))) %>%
            filter(concatid %in% data_preop$concatid)
...
var_dict <- openxlsx::read.xlsx(paste0(loading_dir,"/MCSQI variables (1).xlsx")
                                , sheet = 1
                                , na.strings = ".")
anat_intra_variables <- var_dict$Variable.Name[var_dict$Variable.Category %in% c('intraoperative','anatomical')]
data <- data %>% dplyr::select(one_of(c('concatid', anat_intra_variables)))
