...
# data_inter = t(apply(data_full[3:ncol(data_full)], 1, combn, 2, prod))
# colnames(data_inter) = paste("Inter.V", combn(3:ncol(data_full), 2, paste, collapse=":V"), sep="")
# data_inter2 = data.frame(data_inter)
# data_full = cbind(data_full, data_inter)
# write.csv(data_full, file = paste0(work_dir,'/csv_files/data_full_interaction_terms','.csv'))

# gsub(colnames(data_inter))
# grep(colnames(data_inter)[1], is.numeric)
# !is.na(as.numeric(colnames(data_inter)[1]))
# expand.grid(colnames(data_inter)))

data_full = read.csv(file = paste0(work_dir,'/csv_files/data_full_sts_interaction_terms','.csv')
...
#for (i in 1:2){
  ...
  sub.dir = paste0('cpvntlng/interaction')
