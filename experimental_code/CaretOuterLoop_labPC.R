...
#foreach::foreach(i = 1:5) %dopar% {
  ...
  sub.dir = paste0('cpvntlng/unstack')
  ...
  # glmnet:
  modelGlmnet <- caret::train(label~., data=data, method="glmnet", weights = model_weights, metric = 'ROC',trControl=control)
  save(modelGlmnet,  file = paste(saving_dir, '/modelGlmnet_fold_',i, '.Rdata', sep = ''))
  
  # xgboost:
  modelxgboost <- caret::train(label~., data=data, method="xgbTree", weights = model_weights, metric = 'ROC',trControl=control)
  save(modelxgboost,  file = paste(saving_dir, '/modelxgboost_fold_',i, '.Rdata', sep = ''))

  # ranger:
  start_time = Sys.time()
  modelranger = caret::train(label~., data=data, method="ranger", weights = model_weights, metric = 'ROC', trControl=control)
  ...
  results <- resamples(list(#XGBoost_Bayesian_Optimization = modelxgboost_Bayesian
                            ...
                            ,Glmnet_AuotoTuning = modelGlmnet
                            #,XGBoost_AutoTuning = modelxgboost

                            #,RFranger_AutoTuning = modelranger
                            ))
  saving_dir = 'U:/Hieu/Research_with_CM/cv_surgery/rdata_files/cpvntlng/unstack'
