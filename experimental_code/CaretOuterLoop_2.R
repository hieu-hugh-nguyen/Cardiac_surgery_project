...
trainingid.all = get(load(paste0(loading_dir,
                                   '/all_training_id_outerloop_cpvntlng.RData'
)))
...
#foreach::foreach(i = 1:5) %dopar% {
  ...
  model.prediction_rf.obj = ROCR::prediction(model.predicted.prob_rf[,2], test.data$label)
  ...
  test.auc = c(glm.auc, glmnet.auc, xgboost.auc, rf.auc
               #, xgboost_bayes.auc
               )
